...
/**
 * Prevent the BOT attack on displayed e-mail address
 */
...
/**
 * Remove "Order again" button from the purchase summary page
 */
...
/**
 * Hide the Wootheme alert
 */
	remove_action( 'admin_notices', 'woothemes_updater_notice' );

/**
 * Set Woocommerce PDF Voucher dpi to 300
 */
	function wc_pdf_product_vouchers_image_dpi( $dpi, $voucher ) {

	    // can use $voucher->get_image_id() to check for a specific attachment id
	    return 300;
	}
	add_filter( 'wc_pdf_product_vouchers_voucher_image_dpi', 'wc_pdf_product_vouchers_image_dpi', 10, 2 );

/**
 * Add custom field to Woocommerce PDF Voucher
 * Thanks to https://gist.github.com/justinstern/7109574
 */
/** HASH **/
	add_filter( 'wc_pdf_product_vouchers_voucher_fields', 'wc_pdf_product_vouchers_add_hash_fields', 10, 2 );
	/**
	 ...
	 */
	function wc_pdf_product_vouchers_add_hash_fields( $voucher_fields, $voucher ) {
		// defining a custom field.  Any number of fields can be added in this manner
		$voucher_fields['qrcode'] = array(
			'position' => array(
				'x'     => '168',   // x position from top left
				'y'     => '45', // y position from top left
				'width'  => '57', // width of field
				'background' => array(255,255,255),
				'color'  => array(0, 0, 0)
			),
			'label'  => 'https://qr.retisenzafrontiere.org/{X}'    // your text here
		);
		$voucher_fields['hash'] = array(
			'type' => 'property',
			'font' => array(
				'family' => 'Roboto', // empty for voucher default, otherwise one of Helvetica, Courier or Times
				'size'   => '9', // empty for voucher default, otherwise font size
		...
		$voucher_fields['legal'] = array(
			'type' => 'property',
			'font' => array(
				'family' => 'Roboto', // empty for voucher default, otherwise one of Helvetica, Courier or Times
				'size'   => '6', // empty for voucher default, otherwise font size
				'style'  => '', // empty for voucher default, otherwise one of 'B', 'I' or 'BI' for bold, italic or bold-italic, respectively
				'color'  => '', // empty for voucher default, otherwise a hex color code, ie '#1e73be'
			),
			'text_align'     => 'J',
			'position' => array(
				...
				'width'  => '900', // width of field
				'height' => '42',  // height of field
			),
			'order'     => '',     // not used
			'label'     => 'Questa tessera è personale e non cedibile, dà diritto a sconti e agevolazioni sulle nostre attività. Per qualsiasi informazione, visita il sito',    // your text here
			'multiline' => true,  // whether the field should be rendered as multiline
		);
		$voucher_fields['site'] = array(
			'type' => 'property',
			'font' => array(
				'family' => 'Roboto', // empty for voucher default, otherwise one of Helvetica, Courier or Times
				'size'   => '10', // empty for voucher default, otherwise font size
				'style'  => 'B', // empty for voucher default, otherwise one of 'B', 'I' or 'BI' for bold, italic or bold-italic, respectively
				'color'  => '', // empty for voucher default, otherwise a hex color code, ie '#1e73be'
			),
			'text_align'     => 'C',
			'position' => array(
				'x1'     => '440',   // x position from top left
				'y1'     => '548', // y position from top left
				'width'  => '100', // width of field
				'height' => '42',  // height of field
			),
			'order'     => '',     // not used
			'label'     => 'retisenzafrontiere.org',    // your text here
			'multiline' => false,  // whether the field should be rendered as multiline
		);
		return $voucher_fields;
	}
	// Setting a value for the custom field upon order creation
	add_action( 'woocommerce_add_order_item_meta', 'wc_pdf_product_vouchers_set_hash_fields', 10, 2 );
...
/** Retrieve data **/
	 ...
	 */
	//function wc_pdf_product_vouchers_set_hash_fields( $item_id, $values ) {
		...
		//woocommerce_add_order_item_meta( $item_id, '_ok', 'ko' );
	//}
